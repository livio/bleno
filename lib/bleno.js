...
Bleno.prototype.startAdvertisingAltBeacon = function(mfgId, beaconId, referenceRssi, mfgReserved, callback) {
	var altBeaconCode = 0xBEAC;
	
	var altBeaconData = new Buffer(26, 'hex');
	
	altBeaconData.writeUInt16BE(mfgId, 0);
	altBeaconData.writeUInt16BE(altBeaconCode, 2);
	for (var i = 0; i < 20; i++) {
		altBeaconData[i+4] = beaconId[i];
	}
	altBeaconData.writeUInt8(referenceRssi, 24);
	altBeaconData.writeUInt8(mfgReserved, 25);
